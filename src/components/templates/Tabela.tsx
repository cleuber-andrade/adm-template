    ...
    return clientes?.map((cliente, i) => {
      return (
        <tr key={cliente.id} className={` ${i % 2 === 0 ? "bg-white bg-opacity-40" : "bg-white bg-opacity-20"}`}>
          ...
          <td className="text-center p-4">{cliente.cpf}</td>
          <td className="text-center p-4">{cliente.carteira}</td>
          <td className="text-center p-4">{cliente.endereco}</td>
          <td className="text-center p-4">{cliente.nascimento}</td>
          <td className="text-center p-4">{cliente.tel}</td>
          <td className="text-center p-4">{cliente.plano}</td>
          <td className="text-center p-4">{cliente.especialidade}</td>
          <td className="text-center p-4">{cliente.doutor}</td>
        </tr>
      )
    })
  }
